...
public class JdbcUserDao implements UserDao {
    ...
    public User saveUser(String userName, String password) {
        ...
        //if i know that i only want one row and one column, i can use queryForObject
        //my first parameter is still the query, but my SECOND paramenter is what type of object I expect back
        ...
        /* SQLRowSet results = jdbcTemplate.queryForRowSet(
                "INSERT INTO users(username, password, salt) VALUES (?, ?, ?) RETURNING id",  userName,
                hashedPassword, saltString);
            if (results.next())
                newId = results.getLong("id");
        */
