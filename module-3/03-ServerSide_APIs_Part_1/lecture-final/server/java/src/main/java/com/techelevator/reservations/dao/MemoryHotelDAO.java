...
public class MemoryHotelDAO implements HotelDAO {
	...
	public List<Hotel> filterByStateAndCity(String state, String city) {
		...
		//go thru my hotels one a time and add it to the new list if it matches
		for (Hotel h : hotels) {
			boolean cityMatch = city==null || h.getAddress().getCity().toLowerCase().contains(city.toLowerCase());
			boolean stateMatch = state==null || h.getAddress().getState().toLowerCase().contains(state.toLowerCase());
